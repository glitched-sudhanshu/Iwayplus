...
class LocationService : Service() {
    ...
    private fun start() {
        val notification = NotificationCompat.Builder(this, LOCATION_ID)
            .setContentTitle("Tracking location...")
            .setContentText("Location: null")
            .setSmallIcon(R.drawable.ic_launcher_background)
            .setOngoing(true) //cant swipe it away

        val notificationManager =
            getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        locationClient.getLocationUpdates(10000L) //interval of 10 sec
            .catch { e ->
                e.printStackTrace()
            }
            .onEach { location ->
                ...
                val broadcastIntent = Intent("LOCATION_UPDATES")
                broadcastIntent.putExtra("latitude", lat)
                broadcastIntent.putExtra("longitude", long)
